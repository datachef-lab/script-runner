...
DEFAULT_BASE_URL = "https://results.indiaresults.com/wb/caluniv/b.a-b.sc-semester-iii-2024/result.asp?rollno="
...
def parse_arguments():
    ...
    parser.add_argument("--base-url", "-u", type=str, default=DEFAULT_BASE_URL,
                        help=f"Base URL for the result page (default: {DEFAULT_BASE_URL})")
...
def main():
    ...
    # Set base URL from arguments
    base_url = args.base_url
    
    timestamp = datetime.now().strftime("%Y-%m-%d %I:%M:%S %p")
    print(f"[{timestamp}] Starting data extraction process...")
    if DEBUG:
        print(f"[{timestamp}] DEBUG mode enabled. HTML responses will be saved in debug_html/ directory")
        os.makedirs("debug_html", exist_ok=True)
    
    # Read input Excel file
    try:
        df_input = pd.read_excel(args.input)
        print(f"[{timestamp}] Found {len(df_input)} roll numbers in the input file: {args.input}")
    except Exception as e:
        print(f"[{timestamp}] Error reading input file: {str(e)}")
        return
    
    # Print the column names to help identify the roll number column
    print(f"[{timestamp}] Columns in the input file: {df_input.columns.tolist()}")
    ...
    print(f"[{timestamp}] Using column '{roll_column}' for roll numbers")
    if reg_column:
        print(f"[{timestamp}] Using column '{reg_column}' for registration numbers")
    if name_column:
        print(f"[{timestamp}] Using column '{name_column}' for names")
    
    # Clean roll numbers
    roll_numbers = [clean_roll_number(roll) for roll in df_input[roll_column].tolist()]
    print(f"[{timestamp}] First 5 roll numbers after cleaning: {roll_numbers[:5]}")
    ...
    print(f"[{timestamp}] Will process roll numbers from index {start_idx} to {end_idx-1}")
    
    for i, roll_no in enumerate(roll_numbers[start_idx:end_idx], start=start_idx):
        timestamp = datetime.now().strftime("%Y-%m-%d %I:%M:%S %p")
        print(f"[{timestamp}] Processing {i+1}/{end_idx} ({100*(i-start_idx+1)/(end_idx-start_idx):.1f}%): Roll No. {roll_no}")
        ...
        try:
            ...
            # Make the request using the base URL from arguments
        ...
        # Print progress
        timestamp = datetime.now().strftime("%Y-%m-%d %I:%M:%S %p")
        print(f"[{timestamp}] Completed {i+1}/{end_idx} ({100*(i-start_idx+1)/(end_idx-start_idx):.1f}%) - Success: {successful}, Failed: {failed}")
    
    timestamp = datetime.now().strftime("%Y-%m-%d %I:%M:%S %p")
    print(f"\n[{timestamp}] ✅ Data extraction complete!")
    print(f"[{timestamp}] ✅ Results saved to: {args.output}")
    print(f"[{timestamp}] ✅ Log saved to: {args.log}")
    print(f"[{timestamp}] ✅ Summary: {successful} successful, {failed} failed out of {end_idx-start_idx} roll numbers")
